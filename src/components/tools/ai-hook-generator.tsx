...
export function AiHookGenerator() {
  ...
  const [style, setStyle] = React.useState("Question")
  ...
  const handleGenerate = async () => {
    ...
    setHooks([])
    try {
      const res = await fetch("/api/tools/ai-hook-generator", {
        method: "POST",
        ...
        <CardContent>
          {hooks.length === 0 && !loading && (
            <div className="text-center py-10 text-muted-foreground text-sm">
              Enter a topic and click generate to see hook ideas
            </div>
          )}
          {loading && (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              <Loader2 className="size-8 animate-spin text-primary" />
              <p className="text-sm text-muted-foreground">Creating attention-grabbing hooks...</p>
            </div>
          )}
          <AnimatePresence>
